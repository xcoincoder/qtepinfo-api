...
class TransactionService extends Service {
  async getTransaction(id) {
    ...
    const {Address: RawAddress} = this.app.qtepinfo.lib
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.qtepinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.qtepinfo.rpc(this.app.config.qtepinfo.rpc)
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.qtepinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.qtepinfo.lib
  ...
  async transformQRC20Transfers(outputs) {
    const TransferABI = this.app.qtepinfo.lib.Solidity.qrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
  ...
  async transformQRC20UnconfirmedTransfers(outputs) {
    const {OutputScript, Solidity} = this.app.qtepinfo.lib
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.qtepinfo.lib
    const {
